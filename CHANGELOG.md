...
- **Pins scm-utils v0.2.36 — unbreaks the per-story accept/merge.** `mergePaired` force-checks-out the merge target past the disposable `.consort`/`.lakebase` runtime-artifact churn (guarded to refuse if a dirty *tracked source* file lives outside the ignore set, so `-f` never discards real code), so `consort-pipeline accept` -> `mergePaired` no longer aborts on that uncommitted state (which had wedged the accept path and capture/replay re-recording). Delivered via the scm-utils v0.2.36 pin.
